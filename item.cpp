...
void spawnItem(){
    for(int i=0; i<MAX_ITEMS; i++){
        if(items[i].timer==0){ // 사용 가능한 슬롯 찾기
            ...
            items[i].timer=ITEM_DURATION; // 아이템 유지 시간 설
            map[y][x]=items[i].type; // 맵 데이터 업데이트
            break;
        }
    }
}
...
void updateItems(){
    for(int i=0; i<MAX_ITEMS; i++){
        if(items[i].timer>0){
            ...
            if(items[i].timer==0){
                map[items[i].y][items[i].x]=0; // 맵에서 지우기
                spawnItem();
            }
        }
    }
...
void applyItem(int type){
    if(type==5){ // Growth Item: 꼬리 쪽에 마디 추가
        ...
        map[tail.y][tail.x]=4; // 4: 뱀 몸통 번호
    }else if(type==6){ // Poison Item: 몸 길이 감소
